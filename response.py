...
def response(wish, action):
    ...
    stop_token = "User"  # anything after this will be removed
    if stop_token in responseText:
        responseText = responseText.split(stop_token)[0].strip()

    words = responseText.split()
    responseText = " ".join(dict.fromkeys(words))
